...
const createNewTeam = (userId, leagueId) => {
  mysql.query('INSERT INTO `league_members` (`user_id`, `league_id`) VALUES (?, ?)', [userId, leagueId], (error, members) => {
    if (error) {
    ...
    mysql.query('INSERT INTO `team` (`league_member_id`, `captain`, `brawler_a`, `brawler_b`, `bs_brawler`, `bs_support`, `support`, `villain`, `battlefield`, `bench_a`, `bench_b`, `bench_c`, `bench_d`, `bench_e`, `week`, `points`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [members[0].id, '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''], (error, results) => {
      if (error) {
        return res.status(500).json({
          ...error,
          action: 'add new team'
...
module.exports.joinLeague = (req, res) => {
  ...
  mysql.query('SELECT * FROM league_members WHERE user_id = ?', [user_id], (error, leagues) => {
    if (error) {
      return res.status(500).json({
        ...error,
        action: 'get league members'
