...
module.exports.getSpecificTeamInfo = async (member_id, userId) => {
  try {
    const member = await mysql(
      'SELECT lm.team_name, lm.points, lm.id, l.name, lm.league_id, l.draft_complete, l.active, l.is_roster_active, l.week as leagueWeek FROM league_members lm, league l WHERE lm.id = ? AND lm.league_id = l.id',
      [member_id]
    );
